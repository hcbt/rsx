...
The primitive is `step()`: one CPU instruction; timers, DMA, and GPU time advance with it. Convenience `run_until_vblank_count(n)` drives goldens, headless `--capture-at`, and the Debugger's "step frame". `run_until_cycle(n)` is Debugger realtime: wall time converted through `CPU_HZ`, never the host DAC. First-proof tests run a fixed number of vblanks then hash the Display area. The first golden is captured from our run after the image is accepted by eye. We do not take goldens from another emulator.
