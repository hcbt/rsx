...
For first proof the Debugger shows: Display area, run/pause, step one instruction, step one frame (one vblank), a log of guest I/O and IRQs, CPU PC and GPRs, GPUSTAT. VRAM viewer, GTE/DMA panels, disassembly, and breakpoints come after. A player mode that hides the Debugger and shows only the Display area comes after the hardware works.
